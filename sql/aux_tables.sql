...
-- only those turn-initial chunks are considered which are actually a response 
-- to a previous one from the interlocutor (rather than a continuation of an
-- interrupted turn); this is formalized as at least 50 percent of the previous 
-- chunk having been completed or another chunk from the same speaker existing 
-- right before that previous one
-- note: 
--     selection of non-adjacent turn exchanges involves randomness so results
--     for local similarity will change if this is run multiple times;
...
-- this initial create inserts all adjacent and almost adjacent pairs;
-- non-adjacent pairs are inserted separately below
SELECT 'p' p_or_x, -- truly adjacent pairs
...
AND    con.start1 + (con.end1 - con.start1) / 2 <= con.start2
-- both chunks have all features
AND    con.has_all_features1
AND    con.has_all_features2
-- pairs are not needed for wizard sessions
AND    con.spk_id1 != 0
AND    con.spk_id2 != 0
...
SELECT 'p' p_or_x, -- almost adjacent pairs, treated as adjacent 
       ...
       -- almost turn-final chunk (almost adjacent to turn-initial chunk below)
       con.chu_id1 chu_id1,
       -- turn-initial chunk
       con.chu_id3 chu_id2,
       NULL rid
...
-- and chunk 1 is from same speaker as chunk 2 (so 3 is really a response to 1)
AND    con.start2 + (con.end2 - con.start2) / 2 > con.start3
